...
class NativeFunction(object):
    def __init__(self, cursor):
        ...
        for arg in cursor.get_arguments():
            ...
            nt = NativeType.from_type(arg.type)
            self.arguments.append(nt)
            nt.argumentName = arg.spelling
    ...
    def generate_code(self, current_class=None, generator=None, is_override=False, is_ctor=False):
                ...
                if config['definitions'].has_key('constructor'):
                    if not is_ctor:
                        ...
                        tpl = Template(config['definitions']['ctor'],
                                    searchList=[current_class, self])
                    self.signature_name = str(tpl)
            if self.is_constructor and (gen.script_type == "spidermonkey"  or gen.script_type == "spidermonkey-ts"):
                if not is_ctor:
        ...
        if not is_ctor:
            apidoc_function_script = Template(file=os.path.join(gen.target,
                                          ...
                                          searchList=[current_class, self])
            if gen.script_type == "spidermonkey" or gen.script_type == "spidermonkey-ts":
...
class Generator(object):
    ...
    def generate_code(self):
        ...
        elif self.script_type == "spidermonkey-ts":
            	docfilepath = os.path.join(docfiledir, self.out_file + "_api.d.ts")
        else:
        ...
        layout_c = Template(file=os.path.join(self.target, "templates", "layout_head.c"),
                            searchList=[self])
        if self.script_type != "spidermonkey-ts":
    ...
    def ts_typename_from_natve(self, namespace_class_name):
        script_ns_dict = self.config['conversions']['ns_map']
        if namespace_class_name.find("std::") == 0:
            if namespace_class_name.find("std::string") == 0:
                return "string"
        ...
        for (k, v) in script_ns_dict.items():
            if namespace_class_name.find(k) >= 0:
                ...
                if namespace_class_name.find("cocos2d::Color4F") == 0:
                    return "cc.Color"
                if namespace_class_name.find("char") == 0:
                    return "string"
                else:
                    return namespace_class_name.replace("*","").replace("const ", "").replace(k,v)
        if namespace_class_name.find("int") == 0 or namespace_class_name.find("long") == 0 or namespace_class_name.find("short") == 0  or namespace_class_name.find("double") == 0  or namespace_class_name.find("float") == 0:
            return "number"
        if namespace_class_name.find("bool") == 0 or namespace_class_name.find("BOOL") == 0:
            return "boolean"
        if namespace_class_name.find("int") == 0:
            return "number"
        if namespace_class_name.find("char") == 0 or namespace_class_name.find("unsigned char") == 0:
            return "string"
        return namespace_class_name.replace("*","").replace("const ", "")
